...
const ItemState = (props) => {
  ...
  const getItems = async () => {
    try {
      ...
      arr[3] = {
        _id: "5e597ae0f1acc234600c2ed911",
  ...
  const getSelectedTable = async (data = "actuator") => {
    ...
    try {
      const result = await axios.get(
        `https://hvac-api.herokuapp.com/api/${data}`
      );
      ...
      for (var k in result.data) {
        arr.push({
          name: k,
          condition: result.data[k].status,
          value: result.data[k].anamoly_value,
          lower: result.data[k].lower_value,
          upper: result.data[k].upper_value,
          date: result.data[k].date,
        });
      }
      dispatch({
  ...
  const setParameters = async (name, lower, upper) => {
    ...
    try {
      const result = await axios.post(
        `https://hvac-api.herokuapp.com/api/setParameters`,
        data
      );
  ...
  const getEnergyOptimization = async (data = "actuator") => {
    ...
    try {
      const result = await axios.get(
        `https://hvac-api.herokuapp.com/api/Optimization`
      );
